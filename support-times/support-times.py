...
class SupportTimes(commands.Cog):
    """
    Operating Hours Ticket System 

    With this plugin you can set certain times where ModMail tickets should be enabled/disabled automatically following operating hours.
    ...
    async def support_times(self, ctx: commands.Context):
        """
        Operating Hours Ticket System  - Schedule enabling/disabling ModMail tickets following operating hours

        What is this?
        With this plugin you can set certain times where ModMail tickets should be enabled/disabled.
    ...
    async def enable_modmail(self):
        ...
        if self.bot.config["dm_disabled"] != DMDisabled.NONE:
            ...
            if self.bot.config["log_channel_id"] is not None:
                ...
                if log_channel:
                    embed = discord.Embed(
                        title="ModMail tickets are now enabled!",
                        description=f"ModMail tickets have been automatically enabled following operating hours.",
